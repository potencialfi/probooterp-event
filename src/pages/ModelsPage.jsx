...
const ModelsPage = ({ models, setModels, triggerToast, handleFileImport, loadAllData, settings }) => {
  ...
  const handleAddModel = async () => { /*...*/ if (!newModel.sku || !newModel.color || !newModel.price) { triggerToast("Заполните поля", 'error'); return; } const gridIdToSave = newModel.gridId ? parseInt(newModel.gridId) : defaultGridId; if (!gridIdToSave) { triggerToast("Нет сетки", 'error'); return; } const priceUSD = convertToUSD(parseFloat(newModel.price) || 0, mainCurrency, settings.exchangeRates); try { const created = await apiCall('/models', 'POST', { ...newModel, price: priceUSD, gridId: gridIdToSave }); setModels([created, ...models]); triggerToast("Добавлено"); setNewModel({ ...initialModel, gridId: gridIdToSave }); } catch(e) { triggerToast(e.message, 'error'); } };
  const handleSaveEdit = async () => { /*...*/ const priceUSD = convertToUSD(parseFloat(editingModel.price) || 0, mainCurrency, settings.exchangeRates); try { const updated = await apiCall(`/models/${editingModel.id}`, 'PUT', { ...editingModel, price: priceUSD, gridId: parseInt(editingModel.gridId) }); setModels(models.map(m => m.id === updated.id ? updated : m)); triggerToast("Сохранено"); setIsEditModalOpen(false); } catch(e) { triggerToast(e.message, 'error'); } };
  const handleDeleteModel = async () => { /*...*/ try { await apiCall(`/models/${confirmDeleteId}`, 'DELETE'); setModels(models.filter(m => m.id !== confirmDeleteId)); setConfirmDeleteId(null); triggerToast("Удалено"); } catch (e) { triggerToast("Ошибка", 'error'); } };
  ...
  return (
    <div className="page-container">
            ...
            <table className="ui-table">
                ...
                {paginatedModels.map(m => (
                    <tr key={m.id} className="ui-tr">
                        ...
                        <td className="ui-td"><span className="ui-text-secondary">{m.color}</span></td>
                        <td className="ui-td"><span className="ui-text-money">{convertPrice(m.price, mainCurrency, settings.exchangeRates)}</span></td>
                        <td className="ui-td"><span className="ui-badge ui-badge-primary">{getGridLabel(m.gridId)}</span></td>
                        <td className="ui-td pr-8 text-right"><div className="ui-actions-group"><Button onClick={() => openEditModal(m)} variant="primary" icon={Edit} className="w-9 h-9 px-0"/><Button onClick={() => setConfirmDeleteId(m.id)} variant="danger" icon={Trash2} className="w-9 h-9 px-0"/></div></td>
                    </tr>
                ))}
                </tbody>
            </table>
      ...
      <Modal title="Редактировать модель" isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} footer={<Button onClick={handleSaveEdit} variant="success">Сохранить</Button>}><div className="space-y-4"><div className="grid grid-cols-2 gap-4"><Input label="Артикул" value={editingModel.sku} onChange={e => setEditingModel({...editingModel, sku: e.target.value})}/><Input label="Цвет" value={editingModel.color} onChange={e => setEditingModel({...editingModel, color: e.target.value})}/></div><div className="grid grid-cols-2 gap-4"><Input label="Цена" type="number" value={editingModel.price} onChange={e => setEditingModel({...editingModel, price: e.target.value})}/><Select label="Сетка" value={editingModel.gridId || ''} onChange={e => setEditingModel({...editingModel, gridId: parseInt(e.target.value)})}>{sizeGrids.map(g => (<option key={g.id} value={g.id}>{g.name}</option>))}</Select></div></div></Modal>
