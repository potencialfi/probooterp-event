...
import { Plus, Edit, Trash2, Search, Download, Upload, DollarSign } from 'lucide-react'; // DollarSign можно убрать из импорта, если он больше нигде не используется
...
const ModelsPage = ({ models, setModels, triggerToast, handleFileImport, loadAllData, setImportResult, settings }) => {
  // Состояния для редактирования (Модалка)
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [editingModel, setEditingModel] = useState(initialModel);
  
  // Состояния для создания (Инлайн форма)
  ...
  const handleAddModel = async () => {
    ...
    // Если сетка не выбрана, берем дефолтную
    const gridIdToSave = newModel.gridId ? parseInt(newModel.gridId) : defaultGridId;

    if (!gridIdToSave) {
        triggerToast("Не выбрана размерная сетка", 'error');
        return;
    }
    ...
    try {
        ...
        // Сброс формы, но сетку можно оставить ту же
        setNewModel({ ...initialModel, gridId: gridIdToSave }); 
    } catch(e) {
        triggerToast(e.message || "Ошибка создания", 'error');
    }
  };

  // --- ЛОГИКА РЕДАКТИРОВАНИЯ (Модалка) ---
  ...
  const handleSaveEdit = async () => {
      ...
      const modelData = { 
          ...editingModel, 
          price: priceUSD,
          gridId: parseInt(editingModel.gridId)
      };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* INLINE ADD FORM */}
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div>
              <Input label="Артикул" value={newModel.sku} onChange={e => setNewModel({...newModel, sku: e.target.value})} placeholder="Арт..." />
          </div>
          <div>
              <Input label="Цвет" value={newModel.color} onChange={e => setNewModel({...newModel, color: e.target.value})} placeholder="Цвет..." />
          </div>
          <div>
              <Input label={`Цена (${mainCurrency})`} type="number" value={newModel.price} onChange={e => setNewModel({...newModel, price: e.target.value})} placeholder="0.00" />
          </div>
          <div>
              <Select label="Сетка" value={newModel.gridId || defaultGridId} onChange={e => setNewModel({...newModel, gridId: parseInt(e.target.value)})}>
                  {sizeGrids.map(g => (
                      <option key={g.id} value={g.id}>{g.name} ({g.min}-{g.max})</option>
                  ))}
              </Select>
          </div>
          <div>
              <Button onClick={handleAddModel} icon={Plus} className="w-full">Добавить</Button>
          </div>
      </div>
      ...
      <div className="relative w-full">
        ...
        <input className="w-full border border-gray-300 p-3 pl-12 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder-gray-400 shadow-sm" placeholder="Поиск по артикулу, цвету или сетке..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <table className="w-full text-left">
            <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                <tr>
                    <th className="p-4 w-[40%]">Артикул</th>
                    <th className="p-4">Цвет</th>
                    <th className="p-4">Цена ({mainCurrency})</th>
                    <th className="p-4 text-sm">Сетка</th>
            ...
            {filteredModels.map(m => (
                <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-blue-600 font-medium">{getGridLabel(m.gridId)}</td>
