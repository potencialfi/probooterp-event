...
const NewOrderPage = ({ clients, setClients, models, setOrders, orders, triggerToast, settings, orderDraft, setOrderDraft, clearOrderDraft }) => {
  ...
  const addToCart = () => { const totalQty = Object.values(sizeQuantities).reduce((a, b) => a + b, 0); if (totalQty === 0) return triggerToast("Укажите количество", 'error'); const note = Object.entries(sizeQuantities).filter(([_, q]) => q > 0).map(([s, q]) => `${s}(${q})`).join(', '); updateDraft({ cart: [...cart, { ...currentM, modelId: currentM.id, qty: totalQty, note, sizes: sizeQuantities, discountPerPair: 0, total: currentM.price * totalQty }] }); setSizeQuantities({}); setSelModel(''); setSearch(''); setShowModelList(false); };
  const addBox = (boxSize) => { const t = currentBoxTemplates[boxSize]; if (!t) return triggerToast(`Нет шаблона`, "error"); setSizeQuantities(p => { const n = { ...p }; Object.entries(t).forEach(([s, q]) => { n[s] = (n[s] || 0) + Number(q); }); return n; }); triggerToast(`+ Ящик ${boxSize}`); };
  const saveOrder = async () => { if (!clientPhone || !clientName) return triggerToast("Клиент?", 'error'); if (!cart.length) return triggerToast("Пусто", 'error'); let cid = selectedClient?.id; try { if (selectedClient) { if (selectedClient.name !== clientName || selectedClient.city !== clientCity || selectedClient.phone !== clientPhone) { const u = await apiCall(`/clients/${selectedClient.id}`, 'PUT', { name: clientName, city: clientCity, phone: clientPhone }); setClients(p => p.map(c => c.id === u.id ? u : c)); updateDraft({ selectedClient: u }); } } else { const s = await apiCall('/clients', 'POST', { name: clientName, city: clientCity, phone: clientPhone }); setClients(p => [...p, s]); cid = s.id; } const d = { date: new Date().toISOString(), clientId: parseInt(cid || selectedClient.id), items: cart, total: totalCartUSD, lumpDiscount: lumpDiscountUSD, payment: { originalAmount: Number(prepayment), originalCurrency: paymentCurrency, rateAtMoment: paymentRate, prepaymentInUSD } }; if(editingId) { await apiCall(`/orders/${editingId}`, 'PUT', d); setOrders(p => p.map(o=>o.id===editingId?{...o, ...d}:o)); triggerToast("Обновлено"); } else { const s = await apiCall('/orders', 'POST', d); setOrders([s, ...orders]); triggerToast("Создано"); } clearOrderDraft(); setShowInvoice(false); } catch(e) { triggerToast(e.message, 'error'); } };
  ...
  return (
    <div className="page-container flex-row pb-6">
      <div className="flex flex-col flex-1 gap-6 min-h-0 overflow-hidden">
        <div className="ui-card shrink-0 z-30 overflow-visible">
          <div className="flex justify-between items-center border-b border-gray-100 pb-3 mb-4"><h3 className="section-title mb-0 border-0 pb-0"><User size={22} className="text-blue-600"/> Клиент</h3>{selectedClient && <button onClick={clearSelectedClient} className="ui-badge ui-badge-neutral hover:bg-red-50 hover:text-red-600 cursor-pointer"><X size={14}/> Сброс</button>}</div>
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-end">
            <div className="md:col-span-6 lg:col-span-4 relative"><Input label="Телефон" icon={Phone} value={clientPhone} onChange={handlePhoneChange} autoComplete="off" />{showClientList && filteredClients.length > 0 && (<div className="absolute z-50 w-full bg-white border border-gray-200 rounded-2xl shadow-2xl mt-2 max-h-60 overflow-y-auto p-2">{filteredClients.map(c => (<div key={c.id} className="p-3 hover:bg-blue-50 cursor-pointer rounded-xl border-b border-gray-50 last:border-0" onClick={() => selectClientSuggestion(c)}><div className="font-bold text-base text-gray-800">{c.phone}</div><div className="text-sm text-gray-500">{c.name} ({c.city})</div></div>))}</div>)}</div>
            {!selectedClient && clientPhone.length > 5 && (<div className="md:col-span-12 lg:col-span-12 order-last lg:order-none ui-alert ui-alert-info"><AlertTriangle size={20} className="shrink-0"/><span>Новый клиент</span></div>)}
        ...
        <div className="ui-card flex-1 flex flex-col min-h-0 relative z-10">
          <div className="shrink-0 pb-4 border-b border-gray-100 mb-4 bg-white z-10"><h3 className="section-title mb-4 border-0 pb-0"><ShoppingCart size={22} className="text-blue-600"/> Товары</h3><div className="relative"><Search className="absolute left-4 top-3.5 text-gray-400" size={20} /><input className="ui-input pl-12" placeholder="Поиск модели..." value={search} onChange={e => { setSearch(e.target.value); setShowModelList(true); }} onFocus={() => setShowModelList(true)}/>{showModelList && search && (<div className="border border-gray-100 rounded-2xl mt-2 max-h-72 overflow-y-auto bg-white shadow-2xl absolute w-full z-20 custom-scrollbar p-2">{filteredM.map(m => (<div key={m.id} onClick={() => { setSelModel(m.id); setSearch(m.sku); setShowModelList(false); }} className="p-3 hover:bg-blue-50 rounded-xl cursor-pointer flex justify-between items-center text-base mb-1 border-b border-gray-50 last:border-0"><div><span className="font-bold text-gray-800">{m.sku}</span> <span className="text-gray-400 mx-2">|</span> <span className="text-gray-600">{m.color}</span></div><span className="ui-badge ui-badge-success">{convertPrice(m.price, mainCurrency, settings.exchangeRates)}</span></div>))}</div>)}</div></div>
          <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
              {currentM ? (
                  <div className="bg-blue-50/30 p-6 rounded-3xl border border-blue-100 h-full flex flex-col">
                      <div className="font-bold text-xl text-blue-900 border-b border-blue-200 pb-3 mb-4 flex justify-between items-center"><span>{currentM.sku} / {currentM.color}</span><span className="ui-badge ui-badge-primary text-base">{convertPrice(currentM.price, mainCurrency, settings.exchangeRates)} {mainCurrency}</span></div>
                      {!currentGrid ? <div className="text-center p-6 bg-yellow-50 text-yellow-700 rounded-xl font-bold">Нет сетки</div> : (
                          <div className="flex-1"><div className="mb-6"><div className="flex justify-between items-center mb-3"><label className="ui-label">Размеры</label><button onClick={() => setSizeQuantities({})} className="text-gray-400 hover:text-red-500 text-sm flex items-center gap-1 font-bold"><Eraser size={16}/> Сброс</button></div><div className="flex flex-wrap gap-3">{sizeRange.map(size => (<div key={size} className="flex flex-col items-center"><span className="text-xs text-gray-500 font-bold mb-1">{size}</span><input type="number" className="w-12 h-12 bg-white border border-gray-200 rounded-xl text-center font-bold text-xl text-gray-800 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all" placeholder="0" value={sizeQuantities[size] > 0 ? sizeQuantities[size] : ''} onFocus={(e) => e.target.select()} onChange={e => setSizeQuantities(p => ({...p, [size]: Number(e.target.value)}))} /></div>))}</div></div><div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-3">{availableBoxSizes.length > 0 ? availableBoxSizes.map(bs => (<button key={bs} onClick={() => addBox(bs)} className="bg-white border-2 border-blue-100 text-blue-700 py-3 rounded-2xl text-sm font-bold hover:bg-blue-50 active:scale-95 transition-all shadow-sm flex justify-center items-center gap-2"><Box size={18}/> {bs}</button>)) : <div className="col-span-4 text-center text-sm text-gray-400 p-4 border-2 border-dashed border-gray-200 rounded-xl">Нет шаблонов</div>}</div></div>
                      )}
      ...
      <div className="w-full lg:w-[32rem] ui-card flex flex-col h-full shrink-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar my-4">
            {cart.map((i, idx) => (
                <div key={idx} className="ui-list-item" onClick={() => { setEditingCartIndex(idx); setSizeQuantities(i.sizes); }}>
                    <div><div className="font-bold text-gray-800 text-lg mb-1">{i.sku} <span className="font-normal text-gray-500">({i.color})</span></div><div className="text-base text-gray-600 font-medium">{i.qty} {getNoun(i.qty, 'пара', 'пары', 'пар')}</div>{i.discountPerPair > 0 && <div className="ui-badge ui-badge-success mt-1">-{convertPrice(i.discountPerPair, mainCurrency, settings.exchangeRates)} /пара</div>}</div>
                    <div className="text-right flex flex-col justify-between items-end"><div className="ui-text-money text-xl">{convertPrice(i.total, mainCurrency, settings.exchangeRates)}</div><button onClick={(e) => {e.stopPropagation(); updateDraft({ cart: cart.filter((_, x) => x !== idx) })}} className="text-red-300 hover:text-red-600 p-2 hover:bg-red-50 rounded-xl transition-colors"><Trash2 size={20}/></button></div>
                </div>
            ))}
